...
function create_through_server(req, res) {
  ...
  /* 
  Below line of code changed to 2.	Tag uploaded images (doesn’t matter which tag)' 
  It fetches the user provided tag from req.body
  */
  // Get image tag
  ...
  cloudinary.uploader
    .upload(imageFile, {
      tags: tag === "" ? "Default Tag" : tag,
      width: 500,
      height: 500
    }) // Set tag to default if now user tag is provided
    .then(function(image) {
  ...
  Photo.count()
    ...
    .then(function(preset) {
      if (!preset.settings.return_delete_token) {
        return cloudinary.api.update_upload_preset(preset_name, {
